...
public class AdminDashboardActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            getSupportFragmentManager().beginTransaction().replace(
                    R.id.fragment_Container_customer, new Fragment_AddCategory()).commit();
    ...
    public boolean onNavigationItemSelected(@NonNull MenuItem item) {
        ...
        if (id == R.id.nav_search_medicine_cust) {
            fragment = new Fragment_AddCategory();
        } else if (id == R.id.nav_cart_cust) {
            fragment = new Fragment_PharmaRegRequest();
        } else if (id == R.id.nav_profile_cust) {
            fragment = new Fragment_AdminReports();
